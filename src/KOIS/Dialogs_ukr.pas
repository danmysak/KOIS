...
{$IF DEFINED(MSWINDOWS)}
  WideStrUtils,
{$IFEND}
  ExtCtrls, Consts_ukr, Dlgs, Math, Themes, ActiveX, StrUtils, MultiMon, HelpIntfs;
...
//TODO: Merge: Investigate if these can be merged
{$IF DEFINED(CLR)}
[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.SafeSubWindows)]
function TOpenDialog.Execute(ParentWnd: HWND): Boolean;
const
...
begin
  ...
  begin
    ...
    //TODO: Find a better way to solve that TSavePictureDialog descends from TOpenDialog
...
//TODO: CLR: Why commented out? Or should they be removed from W32?
//    GetPrinter(DevHandle, hDevNames);
//    hDevMode := CopyData(DevHandle);
    hDevNames := 0;
    hDevMode := 0;
...
{$IFEND}
  ButtonNames: array[TMsgDlgBtn] of string = (
    'Так', 'Ні', 'Гаразд', 'Скасувати', 'Перервати', 'Ще раз', 'Ігнорувати', 'Всі', 'Ні для всіх',
    'Так для всіх', 'Довідка', 'Закрити');
...
begin
  ...
  with Form do
    try
      ...
      CheckPrevious.UpdateHandle(Form.Handle);
